...
class pFedMe(Ditto):
    def __init__(
        ...
        super().__init__(strategy, ckpt_path, server_test, proximity)
    ...
    def aggregate(self):
        ...
        for key in aggregated_weights:
            ...
            delta_sum = torch.zeros_like(
                aggregated_weights[key], dtype=torch.float32, device=self.server.device
            )
            ...
            aggregated_weights[key] += (self.momentum * delta_sum).to(
                aggregated_weights[key].dtype
            )
